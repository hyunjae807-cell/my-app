...
def generate_ai_briefing(news_headlines, portfolio_items, api_key):
    ...
    prompt = f"""
    당신은 대형 증권사의 수석 투자전략가 및 리서치센터장 AI입니다.
    투자자가 보유한 종목과 오늘자 실시간 글로벌/국내 주요 뉴스를 바탕으로, 깊이 있고 전문적인 [모닝 프리미엄 마켓 브리핑]을 상세하게 작성해주세요.
    ...
    [작성 가이드라인 - 분량을 충분히 확보하여 상세히 기술할 것]
    
    ## 1. 글로벌 매크로 & 뉴욕 증시 심층 마감
    - 미 증시 주요 지수(나스닥, S&P500, 필라델피아 반도체) 등락 배경 및 원인 분석
    - 국채 금리, 원/달러 환율, 유가, 연준(Fed) 통화정책 기조가 시장에 미친 영향 상세 서술

    ## 2. 국내 증시(코스피/코스닥) 개장 전 관전 포인트
    - 외국인 및 기관 수급 동향 전망
...
def generate_team_briefing(team_name, sports_type, league, team_news, api_key):
    ...
    prompt = f"""
    당신은 스포츠 전문 분석가 및 전술 칼럼니스트 AI입니다.
    [{sports_type} - {team_name} ({league})] 구단에 대해 단순 일정 나열을 넘어, 팬을 위한 심층 매치 리포트 및 프리뷰를 자세히 작성해주세요.

    [규칙]
    - 모든 경기 일정 및 킥오프 시간은 반드시 한국 시간 (KST) 기준으로 표기할 것.
    - 문장을 지나치게 압축하지 말고, 상황과 배경을 친절하고 상세하게 설명할 것.

    [최신 뉴스 데이터]
    {news_text}

    [작성 항목]
    1. ⚽ 다음 경기 집중 프리뷰: 상대팀 전력 비교, 한국 시간(KST) 정확한 매치 데이 & 킥오프 시간, 홈/원정 여부, 주요 관전 포인트
    2. 📊 최근 경기 리뷰 및 경기력 분석: 스코어, 득점자, 팀의 전술적 완성도 및 아쉬웠던 점
    3. 🩺 선수단 컨디션 & 이적/부상 리포트: 주요 부상 복귀자, 결장 의심자, 감독 인터뷰 핵심 요약
    4. 🏆 시즌 순위 경쟁 및 향후 일정 전망: 리그 내 현 위치와 향후 경기들의 중요도
    """
